...
create table students (
  ...
  -- hardware -- and the only reason the app needs to know is so that somebody
  -- else is made responsible for them. Without this they can only ever appear
  -- in the register as MISSING, which is indistinguishable from a child who
  -- should be there and is not.
  has_phone         boolean not null default true,
  -- This student confirms the phone-less riders dealt to them.
...
create table attendance (
  ...
  -- A monitor's confirmation is its own kind of record: a student saying another
  -- student is aboard. Weaker than a driver's, stronger than nothing, and never
  -- merged with a scan.
  source     text not null default 'scan' check (source in ('scan', 'staff', 'monitor')),
...
create table attendance_absence (
  id          uuid primary key default gen_random_uuid(),
  student_id  uuid not null references profiles on delete cascade,
...
create index attendance_absence_student_idx
  on attendance_absence (student_id, on_date desc);
create index attendance_absence_span_idx
...
-- Was callable by anyone including anon: Postgres grants EXECUTE on a new
-- function to PUBLIC. Anon gets nothing back in practice, but "in practice" is
-- not a policy. Signed-in users still can -- linking a family needs it -- and
-- the enumeration that allows is written up in docs/SECURITY.md.
...
--
-- A staff-created rider has a placeholder email on a reserved TLD, so
-- find_user_by_contact() can never turn one up — which means the self-serve
-- link is unavailable to exactly the families whose children most need someone
-- accounting for them. The office makes it instead.
-- ---------------------------------------------------------------------------
...
-- Links are accepted on insert. See patches/2026-09-03e-auto-link.sql for the
-- cost of that and the two things that make it survivable: it is announced to
-- the student and the office, and either side can undo it in one tap. Drop the
-- trigger to put the handshake back; the policy and the queue below still work.
...
-- attendance_absence: same shape. Writes go through declare_absence() and
-- cancel_absence(), which decide who may speak for whom -- one rule in one
-- place rather than three encoded in a policy expression.
